...
void des(BYTE *dat, BYTE * key, BYTE mode);
...
BOOL CheckNumber(unsigned char *pNumber, int nLen);
BYTE extautk[][16]={{0x31, 0x31, 0x31, 0x31, 0x31, 0x31, 0x31, 0x31, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36},
				{0x35, 0x35, 0x35, 0x35, 0x35, 0x35, 0x35, 0x35, 0x35, 0x35, 0x35, 0x35, 0x35, 0x35, 0x35, 0x35},
				{0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36},
				{0x9E ,0x28 ,0x81 ,0xAC ,0x2B ,0x4E ,0x22 ,0xEB ,0x16 ,0x26 ,0x72 ,0x32 ,0x89 ,0xC1 ,0x16 ,0x6A}},
cryptk[][16]={{0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x56, 0x56, 0x56, 0x56, 0x56, 0x56, 0x56, 0x56},
			{0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x57, 0x57, 0x57, 0x57, 0x57, 0x57, 0x57, 0x57}};
BYTE EncrKey[16],EncrKeyLen;
...
void des(BYTE *dat, BYTE *key1, BYTE mode)
{
	...
	for (i = 0; i < 16; i ++){
		selectbits(dat + 4, E, tempbuf, 48);	/* 膨胀变换 */
		if(mode == 0){
			f = shlc;
			count = i;
		}
...
void extaut(BYTE kindex,BYTE *data){
	BYTE key[16];
	memcpy(key,extautk[kindex],16);
	tri_des(data,key,key+8,0);
}
...
void DesJS(BYTE *dat, BYTE *key, BYTE mode, int datalen, int keylen)
{
	for (int i = 0; i<datalen / 8; i++){
		if (keylen == 8)
			des(dat + i * 8, key, mode);
		else
			tri_des(dat + i * 8, key, key + 8, mode);
	}
